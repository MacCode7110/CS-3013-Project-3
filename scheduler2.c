...
void policy_RR (struct job *head, int analysis)
{
  struct job *currententry = head;
  int currenttimecounter = 0;
  int full = 0;
...
while(completed < count){
  for (int i = 0; i < count; i++) {
//    	if(currententry->arrival - response[i-1] > 2 && currententry != NULL){
//printf("%d, %d\n", currententry->arrival, response[i-1]);fflush(stdout);
//}
